...
def loadMnist(train_data_path, train_labels_path, test_data_path, test_labels_path, normalize = 1, one_hot = 1):
    image = {
        'train' : read_image(train_data_path),
        'test' : read_image(test_data_path)
    }
    label = {
        'train' : read_label(train_labels_path),
        'test' : read_label(test_labels_path)
    }
    if normalize:
        for type in ('train', 'test'):
            image[type] = normalize_image(image[type])
    if one_hot:
        for type in ('train', 'test'):
            label[type] = one_hot_label(label[type])
